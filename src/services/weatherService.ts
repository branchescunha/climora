...
export const getCurrentWeather = async (
  ...
  try {
    ...
    if (axios.isAxiosError<WeatherError>(err)) {
      if (err.response) {
        return { success: false, error: getErrorMessage(err.response.status) };
      } else if (err.request) {
        return {
          success: false,
          error: "Sem conexão como servidor, tente novamente",
        };
      } else {
        return {
          success: false,
    ...
    return { success: false, error: "Erro ao buscar clima" };
  }
};
